...
import { getStats, getRequests, getRequestById, updateRequestStatus, deleteRequest } from '@/services/demande-service'
import type { 
  CreateRequestData, 
  RequestStats, 
  RequestStatus,
...
export const demandeKeys = {
  ...
  list: (filters: Record<string, any>) => [...demandeKeys.lists(), { filters }] as const,
  ...
  track: (trackingNumber: string) => [...demandeKeys.all, 'track', trackingNumber] as const,
}
...
export function useUpdateDemandeStatus() {
  ...
  return useMutation({
    mutationFn: ({ id, status, comment }: { id: string; status: RequestStatus; comment?: string }) =>
      updateRequestStatus(id, status, comment),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: demandeKeys.detail(variables.id) })
...
export function useBulkUpdateDemandeStatus() {
  ...
  return useMutation({
    mutationFn: async ({ ids, status, comment }: { ids: string[]; status: RequestStatus; comment?: string }) => {
      const promises = ids.map(id => updateRequestStatus(id, status, comment))
