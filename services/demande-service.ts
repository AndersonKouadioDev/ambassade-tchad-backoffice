...
async function getAuthHeaders() {
  try {
    ...
    console.log('📝 Token preview:', session.user.accessToken.substring(0, 20) + '...');
...
export async function getRequests(params: {
  ...
  try {
    ...
    const url = `${API_CONFIG.baseURL}/demandes?${searchParams.toString()}`;
...
export async function updateRequestStatus(id: string, newStatus: RequestStatus, reason?: string): Promise<Demande> {
  try {
    const headers = await getAuthHeaders();
    const response = await fetch(`${API_CONFIG.baseURL}/demandes/${id}/status`, {
      method: 'PUT',
      headers,
...
export async function getDemandesList(params: {
  page?: number;
  limit?: number;
  status?: RequestStatus;
  serviceType?: ServiceType;
...
export async function updateDemandeStatus(id: string, newStatus: RequestStatus, reason?: string) {
